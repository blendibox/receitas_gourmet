  ...
  <>
      ...
      <div className="flex flex-wrap gap-4 mb-6">
		...
		<input
          ...
          className="border border-purple-200 px-3 py-1 rounded bg-white"
        />
		</label>
			...
			{produtosVisiveis.map((p, i) => (
			  <div key={p.slug || i} className="bg-white p-4 rounded shadow hover:shadow-md transition">
				...
				<Link alt="ir para loja {p._padronizado?.marca}"
					  href={p._padronizado?.link}
					  className="ml-2 p-2 text-white bg-pink-700 rounded text-sm mt-2 inline-block hover:bg-pink-500"
					>
				  Ver Produto
				</Link>
			  </div>
...
{totalPaginas > 1 && (
  <div className="mt-6 flex flex-wrap justify-center items-center gap-2">
    ...
    <button
      ...
      className="px-3 py-1 border border-pink-700 rounded bg-white text-pink-700 hover:bg-pink-50 disabled:opacity-50"
    >
      ← Anterior
    </button>
    ...
    {currentPage > 3 && (
      <>
        <button onClick={() => setCurrentPage(1)} className="px-3 py-1 border border-pink-700 rounded bg-white text-pink-700 hover:bg-pink-50">
          1
        </button>
    ...
    {Array.from({ length: totalPaginas }, (_, i) => i + 1)
      ...
      .map((num) => (
        <button
          ...
          className={`px-3 py-1 border border-pink-700 rounded ${
            currentPage === num
    ...
    {currentPage < totalPaginas - 2 && (
      <>
        ...
        <button onClick={() => setCurrentPage(totalPaginas)} className="px-3 py-1 border border-pink-700 rounded bg-white text-pink-700 hover:bg-pink-50">
          {totalPaginas}
        </button>
      </>
    )}
    ...
    <select
      ...
      className="ml-4 border border-pink-700 rounded px-2 py-1 bg-white text-pink-700"
    >
