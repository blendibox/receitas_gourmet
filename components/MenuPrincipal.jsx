      ...
      <div className="ml-4 border-l pl-2">
        {Object.entries(subcategorias).map(([nome, dados]) => (
		  ...
		  <button id={nome} title={nome}
			  ...
			  className="block px-2 py-1 text-lg text-white hover:bg-red-800 rounded text-left w-full"
			>
			  {nome}
			</button>
  ...
  return (
    <>
      <button name='abreFechaMenu'
        ...
        className="fixed top-4 left-4 z-[9999] bg-red-600 text-white p-2 rounded-md shadow-md"
      ...
      >
		...
		Ofertas para Você <b className="text-pink-500">✱</b>
		</div>
        <input
          type="text"
          className="w-full mb-3 p-2 border rounded border-red-300"
            ...
            Object.entries(categoriasFiltradas).map(([categoria, dados]) => (
              <div key={categoria}>
				...
				<button id={categoria} title={categoria}
				  ...
				  className="block px-2 py-1 text-lg text-gray-800 hover:bg-red-100 rounded text-left w-full"
				>
				  {categoria}
				</button>
                {dados.subcategorias && renderSubcategorias(dados.subcategorias)}
